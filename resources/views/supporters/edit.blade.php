...
<div class="card">
  <div class="card-header bigg text-center">Edit Supporter</div>
  <div class="card-body">
      
      <form action="{{ url('supporters/' .$supporters->id) }}" method="post">
        {!! csrf_field() !!}
        @method("PATCH")
        <input type="hidden" name="id" id="id" value="{{$supporters->id}}" id="id" />
        
        <label class="smol">Name</label></br>
        <input style="color: black !important;" type="text" name="name" id="name" value="{{$supporters->name}}" class="smol form-control"></br>
        
        <label class="smol">Role</label></br>
        <input style="color: black !important;" type="text" name="role" id="role" value="{{$supporters->role}}" class="smol form-control"></br>
        
        <label class="smol">Email</label></br>
        <input style="color: black !important;" type="text" name="email" id="email" value="{{$supporters->email}}" class="smol form-control"></br>
        
        <botton type="submit" class="btn boton me-2">Update</botton>
